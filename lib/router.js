...
function stackLayouts(req, routes, route) {
    ...
    const layouts = routes
        ...
        const output = mustache_1.default.render($.html().replace('{{&gt;', '{{>'), // fix cheerio converting template
        exe, partials);
        if (stacked) {
...
function processFile(req, routes, route, doLayout) {
    ...
    const pageOutput = mustache_1.default.render($.html().replace('{{&gt;', '{{>'), // fix cheerio converting template
    exe, partials);
    if (doLayout && layouts !== '') {
        const $stack = (0, cheerio_1.load)(layouts);
